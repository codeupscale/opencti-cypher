...
const DataTableBody = ({
  ...
  const { resolvePath, rootRef, storageKey, setColumns, useDataTable, useDataTableLocalStorage, variant } = useContext(DataTableContext);
  ...
  const columnSizeVars: ColumnSizeVars = React.useMemo(() => {
    ...
    const clientWidth = containerRef.current!.clientWidth - storedSize - (variant === DataTableVariant.inline ? 10 : 0);
